...
async function uploadFile(file, folder = 'content') {
    try {
        ...
        const fileName = `${currentUser.id}/${folder}/${Date.now()}_${Math.random().toString(36).substring(7)}.${fileExt}`;
...
// Gérer la sélection de fichiers
async function handleFileSelection(files, preview, onUpload, compress) {
    const uploadedFiles = [];
