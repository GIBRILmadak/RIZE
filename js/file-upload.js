...
async function uploadFile(file, folder = 'content') {
    try {
        ...
        if (!isImage && !isVideo) {
            throw new Error('Type de fichier non supporté. Utilisez JPG, PNG, GIF, WebP, MP4 ou WebM.');
        }
        ...
        if (error) {
            ...
            if (error.statusCode === 400) {
                throw new Error('Fichier invalide ou bucket non trouvé. Vérifiez la configuration Supabase.');
...
async function uploadMultipleFiles(files, folder = 'content', onProgress = null) {
    const results = [];
...
function initializeFileInput(inputId, options = {}) {
    ...
    input.addEventListener('change', async (e) => {
        ...
        await handleFileSelection(files, preview, onUpload, compress, { validate });
    });
...
// Gérer la sélection de fichiers
async function handleFileSelection(files, preview, onUpload, compress, options = {}) {
    const uploadedFiles = [];
    const validate = options.validate;
...
function validateFile(file) {
    ...
    // Vérifier la taille
    if (file.size > MAX_FILE_SIZE) {
        errors.push(`Fichier trop volumineux (max ${MAX_FILE_SIZE / 1024 / 1024}MB)`);
    }
