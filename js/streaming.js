...
async function setupBroadcasterMedia() {
    try {
        ...
        if (video) {
            ...
            video.muted = true; // Couper le son local pour éviter l'écho
            video.play();
...
async function initializeStreamPage(streamId) {
    ...
    if (currentUser && currentStream && currentUser.id === currentStream.user_id) {
