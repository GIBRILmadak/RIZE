...
function renderArcDetails(arc, followersCount, isFollowing, content) {
        ...
        if (arc.status === 'in_progress') {
            actionButtons = `
                ...
                <div style="display:flex; gap:0.5rem; justify-content:center; flex-wrap:wrap;">
                    ...
                    <button class="btn btn-ghost" onclick="openEditArcModal(window.currentArc)">Modifier</button>
    ...
    const contentHtml = content && content.length > 0 
        ? `<div class="arc-content-grid">${content.map(c => createContentCardSimple(c)).join('')}</div>`
...
function createContentCardSimple(content) {
