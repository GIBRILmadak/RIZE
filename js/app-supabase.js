...
async function toggleCourage(contentId, btnElement) {
    ...
    const allCourageButtons = document.querySelectorAll(
        `.courage-btn[data-content-id="${contentId}"], .courage-btn`,
    );
    allCourageButtons.forEach((btn) => {
        ...
        if (
            dataId === String(contentId) ||
            (onclickStr && onclickStr.includes(`'${contentId}'`))
        ) {
            ...
            entries.forEach((entry) => {
                ...
                if (entry.isIntersecting) {
                    ...
                    if (contentId && !entry.target.dataset.viewed) {
                        ...
                        if (viewCountSpan) {
                            const current =
                                parseInt(viewCountSpan.textContent) || 0;
            ...
            entries.forEach((entry) => {
                if (entry.isIntersecting) {
                    ...
                    video
                        ...
                        .catch((error) => {
                            console.log(
                                "Autoplay bloqué, attente interaction utilisateur:",
                                error,
                            );
                        });
                } else {
